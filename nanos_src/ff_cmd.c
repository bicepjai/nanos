...
int md5win_cmd(int argc, char **argv) {
	...
	if(argc == 2) {
		...
		if(fSource) {
			...
			cons_printf("Could not open file.\n");
		}
	} else {
		cons_printf("Usage: %s [filename]\n", argv[0]);
	}
	...
	if(argc == 3) {
		...
		if(fSource) {
			...
			if(fDest) {
				...
				FF_Close(fSource);
				cons_printf("Could not open destination file.\n");
			}
		...
		if(fSource) {
			...
			if(fDest) {
				...
				do{
					...
					cons_printf("%3.0f%% - %10ld Bytes Copied, %7.2f Kb/S\r", ((float)((float)ftell(fSource)/(float)SourceSize) * 100), ftell(fSource), transferRate);
				}while(BytesRead > 0);
			...
			cons_printf("Could not open source file.\n");
		}
...
int view_cmd(int argc, char **argv, FF_ENVIRONMENT *pEnv) {
	...
	if(argc == 2) {
		...
		if(f) {
			cons_printf("//---------- START OF FILE\n");
			while(!FF_isEOF(f)) {
				c = FF_GetC(f);
				if(c >= 0) {
					cons_printf("%c", (FF_T_INT8) c);
				} else {
					cons_printf("Error while reading file: %s\n", FF_GetErrMessage(c));
					FF_Close(f);
					return -3;
				}
			}
	...
	if(argc == 2) {
		...
		if(fDest) {
			...
			while(i < pEnv->pIoman->pPartition->TotalSectors) {
				...
				cons_printf("%d%% Complete. (%ld of %ld Sectors read)\r", (int)(((float)i / (float)pEnv->pIoman->pPartition->TotalSectors) * (float)100.0), i,  pEnv->pIoman->pPartition->TotalSectors);
			}
			...
			cons_printf("Couldn't open the destination file!\n");
		}
	} else {
		cons_printf("Usage: %s [filename.img]\n", argv[0]);
	}
...
/*int mkwinfile_cmd(int argc, char **argv) {
	...
	if(argc == 5) {
		...
		if(f) {
			...
			cons_printf("Error opening file\n");
		}
		
	} else {
		cons_printf("Generates a Windows File filled with 32-bit integers.\n\n");
		cons_printf("Usage: %s [Element Size] [Elements] [Multiplier] [filename]\n\n", argv[0]);
		cons_printf("E.g. a 1Mb File, \tFullFAT\\>%s 1024\t 1024\t 1\t 1m.dat\n", argv[0]);
		cons_printf("E.g. a 2Mb File, \tFullFAT\\>%s 1024\t 1024\t 2\t 2m.dat\n", argv[0]);
		cons_printf("E.g. a 2Mb File, \tFullFAT\\>%s 1024\t 1024\t 10\t 10m.dat\n", argv[0]);
...
int run_cmd(int argc, char **argv) {
	...
	if(argc > 1) {
		...
		cons_printf("Usage: %s [command line]\n", argv[0]);
	}
	return 0;
}
...
//	char lines[25][81]; // Screen Buffer
	...
	cons_printf(" HexView 0.2 for FFTerm and FullFAT\n");
	cons_printf(" By James Walmsley\n");
	cons_printf("--------------------------------------------------------\n");
	cons_printf("                         LOADING                        \n");
	usleep(1000);
	...
	if(argc == 2) {
		...
		if(f) {
			...
			cons_printf("Couldn't open file!\n");
		}
	} else {
		cons_printf("No file specified!\n");
	}
