...
// is set to the capacity of the char queue
   //terminals[term_num].out_sid = SemInit(CHAR_Q_SIZE); // a circular q, capacity CHAR_Q_SIZE
...
void StdoutChar(int term_num, u8 ch)
{
...
// LSR_TSRE Line Status Reg Xmit+Shift Regs Empty
   ...
   //outportb(terminals[term_num].io_base + FIFO, FIFO_RCV_RESET);
...
{
    u8 ch;
    TerminalInit_polledio(FT_TERM);
   while(1) // loop of service, servicing shell_pid only actually
   {
      ch = get_serial_char_polledio();
      printf("_%2.0x(%c)_\n",ch,ch);
   }
}
...
u8 get_serial_char_polledio() {
  u8 status;
  printf("=>");
   while(!(status = inportb(terminals[FT_TERM].io_base + LSR) & 0x01)){
     //printf("%2.0x_",status);
   }
   return (inportb(terminals[FT_TERM].io_base + DATA) & 0xFF);
}

// Fill a string buffer with char data. Depending on echo_mode, can read
// up to capacity of str, or till NEWLINE char (null termination added)
/*char StdinChar_polledio(int term_num)
{
;
}*/
...
u8 DeCharQ(char_q_t *p) // return 0/null if queue is empty
{
   u8 ch;
...
void EnCharQ(u8 ch, char_q_t *p) // append ch to p->q[tail] if queue has space
{
   if(CharFullQ(p))
   {
      //cons_printf("Char queue is full, can't enqueue!\n");
